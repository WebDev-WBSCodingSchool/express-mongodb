...
export const getUsers = asyncHandler(async (req, res) => {
  const users = await client.db(databaseName).collection(collectionName).find().toArray();
  console.log(users);
...
export const createUser = asyncHandler(async (req, res) => {
  ...
  const found = await client.db(databaseName).collection(collectionName).findOne({ email });
...
export const getUserById = asyncHandler(async (req, res) => {
  ...
  const user = await client.db(databaseName).collection(collectionName).findOne({ _id: objectID });
